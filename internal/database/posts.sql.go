...
SELECT id, created_at, updated_at, title, url, description, published_at, feed_id FROM posts WHERE feed_id in (SELECT feed_id FROM users_feeds WHERE user_id = $1)
